...
pub unsafe fn scandir(path: *const c_char, namelist: *mut *mut *mut dirent) -> int {
    ...
    unsafe {
        ...
        for file in &uds_files {
            let entry_ptr: *mut dirent =
                posix::malloc(core::mem::size_of::<dirent>()) as *mut dirent;
            let entry = &mut *entry_ptr;

            entry.d_name[..file.len()]
                .copy_from_slice(
                    core::mem::transmute::<&[u8; PATH_LENGTH], &[i8; PATH_LENGTH]>(file),
                );
        ...
        *namelist = posix::malloc(core::mem::size_of::<*mut dirent>() * number_of_files)
            as *mut *mut dirent;
...
pub unsafe fn mkdir(pathname: *const c_char, mode: mode_t) -> int {
    let (dir_created, _) = unsafe {
        win32call! { CreateDirectoryA(pathname as *const u8, core::ptr::null::<SECURITY_ATTRIBUTES>())   ,
