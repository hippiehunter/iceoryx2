...
use windows_sys::Win32::{
    Foundation::{
        ...
        ERROR_BAD_LENGTH, ERROR_CURRENT_DIRECTORY, ERROR_DEV_NOT_EXIST, ERROR_DIR_NOT_EMPTY,
        ERROR_FILE_EXISTS, ERROR_FILE_NOT_FOUND, ERROR_FILE_TOO_LARGE, ERROR_HANDLE_DISK_FULL,
        ERROR_INVALID_ACCESS, ERROR_INVALID_BLOCK, ERROR_INVALID_DATA, ERROR_INVALID_HANDLE,
        ERROR_LOCK_VIOLATION, ERROR_NOT_ENOUGH_MEMORY, ERROR_NOT_READY, ERROR_OUTOFMEMORY,
        ERROR_PATH_NOT_FOUND, ERROR_READ_FAULT, ERROR_SECTOR_NOT_FOUND,
        ERROR_SHARING_BUFFER_EXCEEDED, ERROR_SUCCESS, ERROR_TOO_MANY_OPEN_FILES, ERROR_WRITE_FAULT,
        ERROR_WRITE_PROTECT, WIN32_ERROR,
    },
    Networking::WinSock::{
        WSA_ERROR, WSA_INVALID_HANDLE, WSA_INVALID_PARAMETER, WSA_IO_INCOMPLETE, WSA_IO_PENDING,
        WSA_NOT_ENOUGH_MEMORY, WSAEACCES, WSAEADDRINUSE, WSAEADDRNOTAVAIL, WSAEBADF,
        WSAECONNABORTED, WSAECONNREFUSED, WSAECONNRESET, WSAEHOSTUNREACH, WSAEINTR, WSAEINVAL,
        WSAEISCONN, WSAEMFILE, WSAEMSGSIZE, WSAENETDOWN, WSAENETRESET, WSAENETUNREACH, WSAENOBUFS,
        WSAENOPROTOOPT, WSAENOTSOCK, WSAEPROTONOSUPPORT, WSAEPROTOTYPE, WSAETIMEDOUT,
        WSAEWOULDBLOCK,
    },
};
...
pub unsafe fn system_error_code_to_errno(value: WIN32_ERROR) {
    match value {
        ...
        ERROR_DIR_NOT_EMPTY => Errno::set(Errno::ENOTEMPTY),
        ERROR_ARENA_TRASHED
        | ERROR_INVALID_BLOCK
        | ERROR_WRITE_FAULT
        | ERROR_READ_FAULT
...
pub unsafe fn wsa_to_errno(value: WSA_ERROR) {
    match value {
        ...
        WSAENOTSOCK => Errno::set(Errno::EBADF),
   ...
   {$call:expr, ignore $($error:ident),*} => {
        {
            ...
            if last_error != 0 {
                $crate::os::posix::win32_call::system_error_code_to_errno(last_error);
                match last_error {
                    $($error => ()),*,
                    _ => {
                        #[cfg(feature = "std")]
    ...
    {winsock $call:expr, ignore $($error:ident),*} => {
        {
            ...
            if last_error != 0 {
                $crate::os::posix::win32_call::wsa_to_errno(last_error);
                match last_error {
                    $($error => ()),*,
                    _ => {
                        #[cfg(feature = "std")]
