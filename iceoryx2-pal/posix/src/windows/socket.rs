...
pub unsafe fn setsockopt(
    ...
    let socket_handle = match HandleTranslator::get_instance().get(socket) {
        Some(FdHandleEntry::Socket(mut s)) => {
            if option_name == SO_RCVTIMEO || option_name == SO_SNDTIMEO {
                unsafe { fcntl_int(socket, F_SETFL, O_NONBLOCK) };
                if option_name == SO_RCVTIMEO {
                    s.recv_timeout = unsafe { Some(*(option_value as *const timeval)) };
                } else if option_name == SO_SNDTIMEO {
                    s.send_timeout = unsafe { Some(*(option_value as *const timeval)) };
                }
    ...
    let (sock_opt_result, _) = unsafe {
        win32call! {winsock windows_sys::Win32::Networking::WinSock::setsockopt(socket_handle, level, option_name, option_value as *const u8, option_len as _)}
    };
...
pub unsafe fn getsockopt(
    ...
    let (sock_opt_result, _) = unsafe {
        win32call! {winsock windows_sys::Win32::Networking::WinSock::getsockopt(socket_handle, level, option_name, option_value as *mut u8, option_len as *mut i32)}
    };
    if sock_opt_result == SOCKET_ERROR {
        return -1;
    }
...
pub unsafe fn accept(socket: int, address: *mut sockaddr, address_len: *mut socklen_t) -> int {
    ...
    let (socket, _) = unsafe {
        win32call! {winsock windows_sys::Win32::Networking::WinSock::accept(socket_handle, address.cast(), address_len.cast())}
    };
...
pub unsafe fn listen(socket: int, backlog: int) -> int {
    ...
    let (listen_result, _) = unsafe {
        win32call! {winsock windows_sys::Win32::Networking::WinSock::listen(socket_handle, backlog as _)}
    };
...
pub unsafe fn connect(socket: int, address: *const sockaddr, address_len: socklen_t) -> int {
    match HandleTranslator::get_instance().get(socket) {
        Some(FdHandleEntry::Socket(s)) => {
            let (connect_result, _) = unsafe {
                win32call! {winsock windows_sys::Win32::Networking::WinSock::connect(s.fd, address as *const SOCKADDR, address_len as _)}
            };
        ...
        Some(FdHandleEntry::UdsDatagramSocket(s)) => {
            if !HandleTranslator::get_instance().contains_uds(unsafe {
                (*(address as *const sockaddr_un)).sun_path.as_ptr().cast()
            }) {
            ...
            let udp_address = unsafe { create_uds_address(port) };

            let (connect_result, _) = unsafe {
                win32call! {winsock windows_sys::Win32::Networking::WinSock::connect(s.fd, (&udp_address as *const sockaddr_in) as *const SOCKADDR, core::mem::size_of::<sockaddr_in>() as _)}
            };
            if connect_result == SOCKET_ERROR {
                return -1;
            }
...
pub unsafe fn socket(domain: int, socket_type: int, protocol: int) -> int {
    ...
    if domain == PF_UNIX as _ && socket_type == SOCK_DGRAM {
        let (socket, _) = unsafe {
            win32call! { winsock windows_sys::Win32::Networking::WinSock::socket(PF_INET as _, SOCK_DGRAM, protocol)}
        };
        ...
        let (socket, _) = unsafe {
            win32call! { winsock windows_sys::Win32::Networking::WinSock::socket(domain, socket_type, protocol)}
        };
...
pub unsafe fn sendto(
    ...
    match HandleTranslator::get_instance().get(socket) {
        Some(FdHandleEntry::Socket(s)) => {
            let (bytes_sent, _) = unsafe {
                win32call! {winsock windows_sys::Win32::Networking::WinSock::sendto(s.fd, message as *const u8, length as _, flags, dest_addr as *const SOCKADDR, dest_len as _) }
            };
        ...
        Some(FdHandleEntry::UdsDatagramSocket(s)) => {
            let (bytes_sent, _) = unsafe {
                win32call! {winsock windows_sys::Win32::Networking::WinSock::send(s.fd, message as *const u8, length as _, flags)}
            };
...
pub unsafe fn recvfrom(
    ...
    match HandleTranslator::get_instance().get(socket) {
        Some(FdHandleEntry::Socket(s)) => {
            let (bytes_received, _) = unsafe {
                win32call! {winsock windows_sys::Win32::Networking::WinSock::recvfrom(s.fd, buffer as *mut u8, length as _, flags, address as *mut SOCKADDR, address_len as _) }
            };
        ...
        Some(FdHandleEntry::UdsDatagramSocket(s)) => {
            if let Some(mut timeout) = s.recv_timeout {
                ...
                loop {
                    ...
                    let (number_of_triggered_fds, _) = unsafe {
                        win32call! {select(
                            (s.fd + 1) as _,
                            &mut read_set,
...
pub unsafe fn getsockname(socket: int, address: *mut sockaddr, address_len: *mut socklen_t) -> int {
    ...
    let (sock_name_result, _) = unsafe {
        win32call! {winsock windows_sys::Win32::Networking::WinSock::getsockname(socket_fd, address as *mut SOCKADDR, address_len as _) }
    };
