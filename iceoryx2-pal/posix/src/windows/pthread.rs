...
unsafe fn barrier_wait(barrier: &Barrier) {
    unsafe {
        barrier.wait(
            ...
            |atomic| {
                win32call! { WakeByAddressAll((atomic as *const AtomicU64).cast()) };
            },
        );
    }
}
...
pub unsafe fn pthread_join(thread: pthread_t, retval: *mut *mut void) -> int {
    ...
    let (has_closed, error_code) = unsafe {
        win32call! { CloseHandle(thread.handle) }
    };
    if has_closed == FALSE {
...
pub unsafe fn pthread_self() -> pthread_t {
    ...
    (thread.handle, _) = unsafe {
        win32call! { GetCurrentThread() }
    };
    (thread.id, _) = unsafe {
        win32call! { GetCurrentThreadId() }
    };
...
pub unsafe fn pthread_setname_np(thread: pthread_t, name: *const c_char) -> int {
    let wide_name = unsafe { c_string_to_wide_string(name) };
    unsafe {
...
pub unsafe fn pthread_getname_np(thread: pthread_t, name: *mut c_char, len: size_t) -> int {
    ...
    let name_str = std::ffi::OsString::from_wide(unsafe {
        core::slice::from_raw_parts(wide_name, c_wide_string_length(wide_name))
    })
    .into_string()
...
pub unsafe fn pthread_setaffinity_np(
    ...
    unsafe {
        let (has_set_affinity, _) = win32call! { SetThreadAffinityMask(thread.handle, usize::from_ne_bytes((*cpuset).__bits) )};
    ...
    unsafe {
        match (*lock).lock {
            ...
            RwLockType::PreferWriter(ref l) => l.unlock(
                ...
                |atomic| {
                    WakeByAddressAll((atomic as *const AtomicU64).cast());
                },
            ),
            _ => {
                return Errno::EINVAL as _;
    ...
    unsafe {
        match (*lock).lock {
            ...
            RwLockType::PreferWriter(ref l) => l.write_lock(
                ...
                |atomic| {
                    win32call! { WakeByAddressSingle((atomic as *const AtomicU64).cast()) };
                },
                |atomic| {
                    WakeByAddressAll((atomic as *const AtomicU64).cast());
                },
        ...
        if (*mtx).track_thread_id {
            ...
            if ((*mtx).mtype & PTHREAD_MUTEX_ROBUST) != 0 {
                ...
                if owner != 0 {
                    let mut exit_code = 0;
                    let is_still_active =
                        GetExitCodeThread(owner as _, &mut exit_code) == STILL_ACTIVE;
