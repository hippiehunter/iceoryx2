...
pub enum IamRequest {
    ...
    /// Register the (sender -> receiver) connection-channel handle created by a producer port.
    ///
    /// The producer creates an anonymous zero-copy connection locally and sends the
    /// handle to the IAM server for brokering to the specific receiver. Channels are
    /// keyed by the `(sender_port_id, receiver_port_id)` pair since N receivers imply
    /// N distinct connection channels for a single producer.
    RegisterChannel {
        /// The service the ports belong to.
        service_id: ServiceHash,
        /// The producer (sender) port identifier.
        sender_port_id: u128,
        /// The consumer (receiver) port identifier.
        receiver_port_id: u128,
        /// The size of the connection channel in bytes (advisory; the consumer derives
        /// the true size from the handle).
        channel_size: usize,
        /// The number of handles that will follow this message.
        handle_count: usize,
    },

    /// Request the connection-channel handle for a specific (sender, receiver) pair.
    ///
    /// Consumers (subscriber/client) use this to obtain the handle to the connection
    /// channel a producer created for them, brokered through the IAM server.
    RequestChannelHandle {
        /// The service the ports belong to.
        service_id: ServiceHash,
        /// The producer (sender) port whose channel handle is requested.
        sender_port_id: u128,
        /// The consumer (receiver) port requesting the channel handle.
        receiver_port_id: u128,
    },

    /// Register the management-segment handle of a producer port's resizable memory.
    ///
    /// A producer's resizable shared memory owns a per-port singleton **management**
    /// segment (anonymous, memfd-backed). The producer creates it locally and sends the
    /// handle to the IAM server so the management segment can be brokered to authorized
    /// consumers. Keyed by the single producer `port_id` (like data segments, unlike the
    /// pair-keyed connection channels) since a producer has exactly one management segment.
    RegisterMgmtSegment {
        /// The service the port belongs to.
        service_id: ServiceHash,
        /// The producer port identifier.
        port_id: u128,
        /// The size of the management segment in bytes.
        mgmt_size: usize,
        /// The number of handles that will follow this message.
        handle_count: usize,
    },

    /// Request the management-segment handle for a producer port.
    ///
    /// Consumers (subscriber/client) use this to obtain the handle to a producer's
    /// resizable-memory management segment, brokered through the IAM server.
    RequestMgmtSegmentHandle {
        /// The service the port belongs to.
        service_id: ServiceHash,
        /// The producer port whose management-segment handle is requested.
        port_id: u128,
    },
}
...
pub enum IamResponse {
    ...
    /// Successful response to a RegisterChannel request.
    RegisterChannelOk,

    /// Successful response to a RequestChannelHandle request — handle available.
    ///
    /// `segment_info.access` is read+write for channels because the consumer's receiver
    /// end writes the connection's completion ring.
    ChannelHandleOk {
        /// Metadata about the connection channel being provided.
        segment_info: SegmentInfo,
        /// The number of handles that will follow this message.
        handle_count: usize,
    },

    /// Response to a RequestChannelHandle request — no channel registered yet.
    ///
    /// The producer has not yet registered the connection channel for this
    /// `(sender, receiver)` pair. The consumer should retry later (edge-triggered rescan).
    ChannelHandleNotFound,

    /// Successful response to a RegisterMgmtSegment request.
    RegisterMgmtSegmentOk,

    /// Successful response to a RequestMgmtSegmentHandle request — handle available.
    ///
    /// `segment_info.access` is read-only for the management segment: the consumer's
    /// `DynamicView` maps it purely as a keep-alive token and never reads or writes it.
    MgmtSegmentHandleOk {
        /// Metadata about the management segment being provided.
        segment_info: SegmentInfo,
        /// The number of handles that will follow this message.
        handle_count: usize,
    },

    /// Response to a RequestMgmtSegmentHandle request — no management segment registered yet.
    ///
    /// The producer has not yet registered its management segment. The consumer should
    /// retry later (edge-triggered rescan).
    MgmtSegmentHandleNotFound,
}
...
mod tests {
    ...
    fn test_iam_response_create_service_ok_serialization_roundtrip() {
        ...
        let service_id = ServiceHash::new::<Sha1>(&service_name, MessagingPattern::PublishSubscribe);
    ...
    #[test]
    fn test_iam_request_register_channel_serialization_roundtrip() {
        use crate::service::messaging_pattern::MessagingPattern;
        use iceoryx2_cal::hash::sha1::Sha1;

        let service_name = ServiceName::new("test/register_channel").unwrap();
        let service_id = ServiceHash::new::<Sha1>(&service_name, MessagingPattern::PublishSubscribe);

        let request = IamRequest::RegisterChannel {
            service_id,
            sender_port_id: 0xAAAA,
            receiver_port_id: 0xBBBB,
            channel_size: 32768,
            handle_count: 1,
        };
        let serialized = postcard::to_allocvec(&request).unwrap();
        ...
        match deserialized {
            IamRequest::RegisterChannel {
                ...
                assert_eq!(deser_id, service_id);
                assert_eq!(sender_port_id, 0xAAAA);
                assert_eq!(receiver_port_id, 0xBBBB);
                assert_eq!(channel_size, 32768);
                assert_eq!(handle_count, 1);
            }
            _ => panic!("Expected RegisterChannel request"),
        }
    }

    #[test]
    fn test_iam_request_request_channel_handle_serialization_roundtrip() {
        use crate::service::messaging_pattern::MessagingPattern;
        use iceoryx2_cal::hash::sha1::Sha1;

        let service_name = ServiceName::new("test/request_channel").unwrap();
        let service_id = ServiceHash::new::<Sha1>(&service_name, MessagingPattern::PublishSubscribe);

        let request = IamRequest::RequestChannelHandle {
            service_id,
            sender_port_id: 7,
            receiver_port_id: 9,
        };
        ...
        match deserialized {
            IamRequest::RequestChannelHandle {
                ...
                assert_eq!(deser_id, service_id);
                assert_eq!(sender_port_id, 7);
                assert_eq!(receiver_port_id, 9);
            }
            _ => panic!("Expected RequestChannelHandle request"),
        }
    }

    #[test]
    fn test_iam_response_register_channel_ok_serialization_roundtrip() {
        let response = IamResponse::RegisterChannelOk;
        let serialized = postcard::to_allocvec(&response).unwrap();
        let deserialized: IamResponse = postcard::from_bytes(&serialized).unwrap();
        assert!(matches!(deserialized, IamResponse::RegisterChannelOk));
    }

    #[test]
    fn test_iam_response_channel_handle_ok_serialization_roundtrip() {
        // Channels are brokered with read+write access.
        let info = SegmentInfo::new(SegmentId::new(0), 32768, AccessRights::read_write());
        let response = IamResponse::ChannelHandleOk {
            segment_info: info,
            handle_count: 1,
        };
        let serialized = postcard::to_allocvec(&response).unwrap();
        ...
        match deserialized {
            IamResponse::ChannelHandleOk {
                ...
                assert_eq!(segment_info, info);
                assert!(segment_info.access().can_read());
                assert!(segment_info.access().can_write());
                assert_eq!(handle_count, 1);
            }
            _ => panic!("Expected ChannelHandleOk response"),
        }
    }

    #[test]
    fn test_iam_response_channel_handle_not_found_serialization_roundtrip() {
        let response = IamResponse::ChannelHandleNotFound;
        let serialized = postcard::to_allocvec(&response).unwrap();
        let deserialized: IamResponse = postcard::from_bytes(&serialized).unwrap();
        assert!(matches!(deserialized, IamResponse::ChannelHandleNotFound));
    }

    #[test]
    fn test_iam_request_register_mgmt_segment_serialization_roundtrip() {
        use crate::service::messaging_pattern::MessagingPattern;
        use iceoryx2_cal::hash::sha1::Sha1;

        let service_name = ServiceName::new("test/register_mgmt_segment").unwrap();
        let service_id = ServiceHash::new::<Sha1>(&service_name, MessagingPattern::PublishSubscribe);

        let request = IamRequest::RegisterMgmtSegment {
            service_id,
            port_id: 0xCAFE,
            mgmt_size: 4096,
            handle_count: 1,
        ...
        let deserialized: IamRequest = postcard::from_bytes(&serialized).unwrap();
        match deserialized {
            IamRequest::RegisterMgmtSegment {
                service_id: deser_id,
                port_id,
                mgmt_size,
                handle_count,
                ...
                assert_eq!(deser_id, service_id);
                assert_eq!(port_id, 0xCAFE);
                assert_eq!(mgmt_size, 4096);
                assert_eq!(handle_count, 1);
            }
            _ => panic!("Expected RegisterMgmtSegment request"),
        }
    }

    #[test]
    fn test_iam_request_request_mgmt_segment_handle_serialization_roundtrip() {
        use crate::service::messaging_pattern::MessagingPattern;
        use iceoryx2_cal::hash::sha1::Sha1;

        let service_name = ServiceName::new("test/request_mgmt_segment").unwrap();
        let service_id = ServiceHash::new::<Sha1>(&service_name, MessagingPattern::PublishSubscribe);

        let request = IamRequest::RequestMgmtSegmentHandle {
            service_id,
            port_id: 55,
        };
        let serialized = postcard::to_allocvec(&request).unwrap();
        ...
        match deserialized {
            IamRequest::RequestMgmtSegmentHandle {
                ...
                assert_eq!(deser_id, service_id);
                assert_eq!(port_id, 55);
            }
            _ => panic!("Expected RequestMgmtSegmentHandle request"),
        }
    }

    #[test]
    fn test_iam_response_register_mgmt_segment_ok_serialization_roundtrip() {
        let response = IamResponse::RegisterMgmtSegmentOk;
        let serialized = postcard::to_allocvec(&response).unwrap();
        let deserialized: IamResponse = postcard::from_bytes(&serialized).unwrap();
        assert!(matches!(deserialized, IamResponse::RegisterMgmtSegmentOk));
    }

    #[test]
    fn test_iam_response_mgmt_segment_handle_ok_serialization_roundtrip() {
        // The management segment is brokered read-only (never read/written by the view).
        let info = SegmentInfo::new(SegmentId::new(0), 4096, AccessRights::read_only());
        let response = IamResponse::MgmtSegmentHandleOk {
            segment_info: info,
            handle_count: 1,
        };
        let serialized = postcard::to_allocvec(&response).unwrap();
        ...
        match deserialized {
            ...
            _ => panic!("Expected MgmtSegmentHandleOk response"),
        }
    }

    #[test]
    fn test_iam_response_mgmt_segment_handle_not_found_serialization_roundtrip() {
        let response = IamResponse::MgmtSegmentHandleNotFound;
        let serialized = postcard::to_allocvec(&response).unwrap();
        let deserialized: IamResponse = postcard::from_bytes(&serialized).unwrap();
        assert!(matches!(
            deserialized,
            IamResponse::MgmtSegmentHandleNotFound
        ));
    }
