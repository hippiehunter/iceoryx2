...
impl<C: CalClient> IamClient<C> {
    ...
    ) -> Result<ServiceHash, IamClientError> {
        self.ensure_active()?;
    ...
    fn handle_create_service_response(response: IamResponse) -> Result<ServiceHash, IamClientError> {
        match response {
    ...
    /// - [`IamClientError::SessionInvalid`] - Client is not in active state
    /// - [`IamClientError::RequestDenied`] - Server denied the request
    pub fn detach(&mut self, service_id: &ServiceHash, port_id: u128) -> Result<(), IamClientError> {
        self.ensure_active()?;
    ...
    // ========================================================================
    // Connection Channel Registration & Request
    // ========================================================================

    /// Registers a producer's connection-channel handle with the IAM server.
    ///
    /// After creating an anonymous zero-copy connection, the producer
    /// (publisher/server) sends the handle to the IAM server so it can be brokered
    /// to the specific receiver identified by the `(sender, receiver)` port pair.
    ///
    /// # Arguments
    ///
    /// * `service_id` - The service the ports belong to
    /// * `sender_port_id` - The producer port that owns the connection channel
    /// * `receiver_port_id` - The consumer port the channel is for
    /// * `channel_size` - The size of the connection channel in bytes (advisory)
    /// * `handle` - The platform handle for the anonymous connection channel
    ///
    /// # Returns
    ///
    /// `Ok(())` once the IAM server has acknowledged the registration.
    pub fn register_channel(
        ...
        match response {
            IamResponse::RegisterChannelOk => Ok(()),
            IamResponse::Denied { .. } => Err(IamClientError::RequestDenied),
            IamResponse::ProtocolError { .. } => Err(IamClientError::ProtocolError),
            _ => Err(IamClientError::ProtocolError),
        }
    ...
    /// Requests the connection-channel handle for a `(sender, receiver)` pair.
    ///
    /// Consumers (subscriber/client) use this to obtain the handle to the connection
    /// channel that a producer created for them, brokered through the IAM server.
    ///
    /// # Arguments
    ///
    /// * `service_id` - The service the ports belong to
    /// * `sender_port_id` - The producer port whose channel handle is requested
    /// * `receiver_port_id` - The consumer port requesting the channel handle
    ///
    /// # Returns
    ///
    /// * `Ok(Some((segment_info, handle)))` if the handle is available
    /// * `Ok(None)` if the producer has not yet registered this channel
    ///   (race condition - retry later)
    /// * Error on communication failure
    pub fn request_channel_handle(
        &mut self,
        service_id: &ServiceHash,
        sender_port_id: u128,
        receiver_port_id: u128,
        ...
        match response {
            ...
            IamResponse::ChannelHandleNotFound => Ok(None),
            IamResponse::Denied { .. } => Err(IamClientError::RequestDenied),
            IamResponse::ProtocolError { .. } => Err(IamClientError::ProtocolError),
            _ => Err(IamClientError::ProtocolError),
        }
    ...
    // ========================================================================
    // Management Segment Registration & Request
    // ========================================================================

    /// Registers a producer's resizable-memory management-segment handle with the IAM server.
    ///
    /// After creating the anonymous management segment of its resizable shared memory, the
    /// producer (publisher/server) sends the handle to the IAM server so it can be brokered
    /// to authorized consumers. Keyed by the single producer `port_id`.
    ///
    /// # Arguments
    ///
    /// * `service_id` - The service the port belongs to
    /// * `port_id` - The producer port that owns the management segment
    /// * `mgmt_size` - The size of the management segment in bytes
    ...
    pub fn register_mgmt_segment(
        ...
        match response {
            IamResponse::RegisterMgmtSegmentOk => Ok(()),
            IamResponse::Denied { .. } => Err(IamClientError::RequestDenied),
            IamResponse::ProtocolError { .. } => Err(IamClientError::ProtocolError),
            _ => Err(IamClientError::ProtocolError),
        }
    ...
    /// Requests the management-segment handle for a producer port.
    ///
    ...
    ///
    /// * `service_id` - The service the port belongs to
    /// * `port_id` - The producer port whose management-segment handle is requested
    ///
    /// # Returns
    ///
    /// * `Ok(Some((segment_info, handle)))` if the handle is available
    /// * `Ok(None)` if the producer has not yet registered its management segment
    ///   (race condition - retry later)
    /// * Error on communication failure
    pub fn request_mgmt_segment_handle(
        &mut self,
        service_id: &ServiceHash,
        port_id: u128,
    ) -> Result<Option<(SegmentInfo, PlatformHandle)>, IamClientError> {
        ...
        match response {
            ...
            IamResponse::MgmtSegmentHandleNotFound => Ok(None),
            IamResponse::Denied { .. } => Err(IamClientError::RequestDenied),
            IamResponse::ProtocolError { .. } => Err(IamClientError::ProtocolError),
            _ => Err(IamClientError::ProtocolError),
        }
...
mod tests {
    ...
    fn test_client_create_service_success() {
        ...
        let expected_service_id =
            ServiceHash::new::<Sha1>(&service_name, MessagingPattern::PublishSubscribe);
    ...
    fn test_client_attach_server_success() {
        ...
        let connection = MockConnection::new();
        let service_name = ServiceName::new("test/service").unwrap();
        let service_id = ServiceHash::new::<Sha1>(&service_name, MessagingPattern::RequestResponse);
