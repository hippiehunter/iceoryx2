...
pub(crate) fn receive_handles_from_client<C: ControlChannelConnection>(
    ...
    // A client always sends its handle(s) immediately after the request that announces them, so a
    // short bounded wait suffices. Bounding it (instead of `receive_handles`'s ~9h
    // `BLOCKING_TIMEOUT`) keeps the single-threaded server's poll loop responsive and, crucially,
    // shutdownable even if a client dies mid-handoff — otherwise the poll thread wedges here and
    // `TypeErasedIamServer::drop`'s `join()` deadlocks.
    const HANDLE_RECEIVE_TIMEOUT: core::time::Duration = core::time::Duration::from_secs(3);
    conn.timed_receive_handles(HANDLE_RECEIVE_TIMEOUT)
        .map_err(|_| IamServerError::HandlePassingFailed)?
